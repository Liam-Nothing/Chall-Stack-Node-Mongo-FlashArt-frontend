...
const fetchSlots = async () => {
  try {
    const token = localStorage.getItem('token')
    const response = await axios.get(`http://localhost:5000/api/slots/available?tatoueurId=${flash.value.id_tatoueur._id}`, {
      headers: {
...
const bookSlot = async (slot) => {
  try {
    const token = localStorage.getItem('token')
    const response = await axios.put(`http://localhost:5000/api/slots/${slot._id}/book`, {}, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
